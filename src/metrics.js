...
const { readObservationsFile } = require('./observation-store')
...
function readObservations(filePath = defaultObservationPath) {
  return readObservationsFile(filePath)
}
...
function isAiAuthored(observation) {
  ...
  return inferAttributionSignals(observation).some((signal) => signal !== 'bot-author')
}

function inferAttributionSignals(observation) {
  if (!observation || typeof observation !== 'object') {
    return []
  }

  if (typeof observation.aiAuthored === 'boolean') {
    return [`maintainer-override:${observation.aiAuthored}`]
  }
  ...
  const labels = Array.isArray(observation.labels) ? observation.labels.map((label) => String(label).toLowerCase()) : []
  const signals = []

  if (/(copilot|gpt|claude|chatgpt|devin|cursor|openai|-ai\b)/.test(author)) signals.push('author-pattern')
  if (/\bautomated\b|\bcopilot\b|\bclaude\b|\bchatgpt\b|\bgpt\b|\bdevin\b|\bcursor\b/.test(title)) signals.push('title-pattern')
  if (/(generated by|co-authored-by|written with|authored with|assisted by|drafted with)/.test(body)) signals.push('body-pattern')
  if (labels.includes('ai') || labels.includes('ai-authored')) signals.push('label-pattern')
  if (author.includes('bot')) signals.push('bot-author')
  return signals
}
...
function pickRecent(items, now, days) {
  return items.filter((item) => {
    ...
    const age = openedAt ? daysBetween(now, openedAt) : null
    return age !== null && age >= 0 && age <= days
  })
}

function isWithinRecentWindow(now, value, days) {
  const date = asDate(value)
  const age = date ? daysBetween(now, date) : null
  return age !== null && age >= 0 && age <= days
}
...
function buildRawSnapshot(options = {}) {
  ...
  const observations = (options.observations || readObservations(options.filePath))
    .map((observation) => normalizeObservation(observation, { isAiAuthored, inferAttributionSignals, asDate }))
  ...
  const recentMergedAi = aiObservations.filter((observation) => {
    ...
    return isWithinRecentWindow(now, observation.mergedAt, 30)
  })
  const recentClosed = observations.filter((observation) => {
    return isWithinRecentWindow(now, observation.closedAt || observation.mergedAt, 14)
  })
...
function buildSnapshot(options = {}) {
  if (Array.isArray(options.observations)) {
    return buildCuratedSnapshot(buildRawSnapshot(options))
  }

  const filePath = options.filePath || defaultObservationPath
  const stat = fs.existsSync(filePath) ? fs.statSync(filePath) : null
  const cacheKey = JSON.stringify({
    filePath,
    now: options.now || '',
    fileMtimeMs: stat ? stat.mtimeMs : null,
  ...
  const curated = buildCuratedSnapshot(buildRawSnapshot(options))
  metricsCache.set(cacheKey, curated, 1500)
  return curated
}

function buildCuratedSnapshot(raw) {
